...
docker-compose build base && docker-compose build wrf_build && docker-compose build wrf_prod && docker-compose build rasp && cd .. && ./scheduled_update.sh -f
...
If you enable Docker BuildKit (e.g. `export DOCKER_BUILDKIT=1` before building), the geog dataset
will now be cached per region and reused across subsequent builds, dramatically reducing rebuild
times when you tweak only the namelist files.
