...
sub output_model_results_hhmm ()
{
  ...
  ## Set IWINDOW to 0 for non-windowed runs
  $IWINDOW = 0;

  # Allow plotting of wrfout_d0[12 ...] according to max_dom in namelist.input file
  # Modified to loop over ALL domains that have PLOT_IMAGE_SIZE defined (not just max_dom)
  ...
  if ($LPRINT>1) { 
    print $PRINTFH "     DEBUG: regionname='$regionname' regionkey='$regionkey' IWINDOW=$IWINDOW max_dom=$max_dom\n";
    print $PRINTFH "     DEBUG: PLOT_IMAGE_SIZE hash keys: " . join(", ", keys %PLOT_IMAGE_SIZE) . "\n";
  }
  
  # Loop over all domains (1 to max_dom) and process those with PLOT_IMAGE_SIZE defined
  for ( my $plot_dom = 1; $plot_dom <= $max_dom; $plot_dom++ ) {
    # DEBUG: Check what value we get
    if ($LPRINT>1) { 
      my $val = $PLOT_IMAGE_SIZE{$regionname}[$IWINDOW][$plot_dom-1];
      print $PRINTFH "     DEBUG: PLOT_IMAGE_SIZE{$regionname}[$IWINDOW][$plot_dom-1] = " . (defined $val ? "'$val'" : "UNDEF") . "\n";
    }
    # Skip domains without PLOT_IMAGE_SIZE defined (index is plot_dom-1)
    if( !defined $PLOT_IMAGE_SIZE{$regionname}[$IWINDOW][$plot_dom-1] || 
        $PLOT_IMAGE_SIZE{$regionname}[$IWINDOW][$plot_dom-1] =~ /^\s*$/ ) {
      if ($LPRINT>1) { print $PRINTFH "     Skipping domain $plot_dom (no PLOT_IMAGE_SIZE defined)\n"; }
      next;
    }
    if ($LPRINT>1) { print $PRINTFH "     Processing domain $plot_dom with PLOT_IMAGE_SIZE=$PLOT_IMAGE_SIZE{$regionname}[$IWINDOW][$plot_dom-1]\n"; }
    
    my $wrf_dom = sprintf "wrfout_d0%d*", $plot_dom ;
    ...
    HHMM: for( my $t = 0; $t <= $#historyhhmmlist; $t++) {
      ...
      $nameregex =  sprintf "wrfout_d0%d_2[0-9][0-9][0-9]-[01][0-9]-[0-3][0-9]_%02d:%02d:00",$plot_dom,${historyhh},${historymm};
  ...
  } # End of loop over domains (plot_dom)
  ...
  # Now check if they were successful - check all domains with PLOT_IMAGE_SIZE
  for ( my $chk_dom = 1; $chk_dom <= $max_dom; $chk_dom++ ) {
    # Skip domains without PLOT_IMAGE_SIZE defined
    if( !defined $PLOT_IMAGE_SIZE{$regionname}[$IWINDOW][$chk_dom-1] || 
        $PLOT_IMAGE_SIZE{$regionname}[$IWINDOW][$chk_dom-1] =~ /^\s*$/ ) {
      next;
    }
    foreach $historyhhmm (@historyhhmmlist) {
      my $logfile = "$RUNDIR/LOG/ncl.out.0$chk_dom.$historyhhmm" ;
      next if( ! -f $logfile );  # Skip if log doesn't exist
      ...
      if( grep /fatal/, $lastline || ! grep /NORMAL END/, $lastline ){    # Fail!
        ...
        $msg1 = "    NCL for domain $chk_dom $historyhhmm FAILED: See $errlogfile";
        if( $LPRINT > 1){ print $PRINTFH $msg1 . "\n" ;}
        if( ( $RUNTYPE eq '-M' || $RUNTYPE eq '-m' || $RUNTYPE eq '-i' ) ) {
          `echo $msg1 | mail -s "NCL FAILURE for $moad - domain $chk_dom $historyhhmm" "$ADMIN_EMAIL_ADDRESS" 2>&1`;
        }
      }
    }
  }
  ...
  if($pfd_flag){
    ...
    `cd $NCLDIR ; faketime '2025-12-01' ncl -n -p < wrf2gm.ncl > $logfile 2>&1` ;
  }
...
#   $imagedir = "$OUTDIR/$moad" ;
    ...
    $ENV{'PROJECTION'}         = 'mercator' ;  # Set map projection for NCL plotting
   ...
   `cd $ENV{'ENV_NCL_OUTDIR'} ; rm -f $paramiddatastring 2>/dev/null`;
    ...
    my $cmd = "cd $NCLDIR ; faketime '2025-12-01' ncl -n -p < wrf2gm.ncl > $logfile 2>&1" ;
    my $childproc = Proc::Background->new("$cmd");
...
### TO ALLOW MID-DAY RESTART WITH FTP-PARALLEL
           ...
           if ( $filestatus{$ifile} == ($iattempt-1) )
           {
             ...
             {
               $gribftpdirectory[1] = sprintf 'gfs.%04d%02d%02d/%02d/atmos',$jyr4,$jmo2,$jda2,$fileanaltimes{$ifile};
               $gribftpdirectory[0] = sprintf 'gfs.%04d%02d%02d/%02d/atmos',$jyr4m1,$jmo2m1,$jda2m1,$fileanaltimes{$ifile};
               $gribftpdirectory[2] = sprintf 'gfs.%04d%02d%02d/%02d/atmos',$jyr4p1,$jmo2p1,$jda2p1,$fileanaltimes{$ifile};
             }
